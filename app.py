...
def summarize(text, user_id=None, max_input_length=1024, summary_type='main'):
    ...
    return summary, None  # Trả về summary và error (None nếu không có lỗi)
...
def api_summarize():
    ...
    summary_type = data.get('summary_type', 'main')

    if not text.strip():
        return jsonify({'error': 'Văn bản nhập vào trống!'}), 400

    try:
        summary, error = summarize(text, user_id, summary_type=summary_type)
        if error:
            return jsonify({'error': error}), 400
        if summary is None:
            return jsonify({'error': 'Không thể tóm tắt văn bản!'}), 500
...
def summarize_article():
    ...
    summary, error = summarize(content, user_id, summary_type=summary_type)
    if error:
        return jsonify({'error': error}), 400
    if summary is None:
        return jsonify({'error': 'Không thể tóm tắt bài viết!'}), 500
